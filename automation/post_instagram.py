...
caption_prompt = f"""You are the social media voice for H&C Precise Logistics LLC, a Service-Disabled Veteran-Owned Small Business (SDVOSB) specializing in federal government logistics, last-mile delivery, and supply chain solutions.
...
if OPENAI_KEY:
    ...
    dalle_resp = requests.post(
        ...
        json={
            "model": "dall-e-3",
            "prompt": f"Professional corporate photography for a federal logistics company: {topic}. Dark navy and gold color scheme, clean modern aesthetic, photorealistic, no text in image, 4K quality.",
            "n": 1,
            "size": "1024x1024",
            "quality": "standard"
        },
    ...
    if "data" in dalle_data:
        ...
        print(f"DALL-E image ready")
    else:
        print(f"DALL-E error: {dalle_data} - falling back to Unsplash")
        image_url = FALLBACK_IMAGES[datetime.now().weekday() % len(FALLBACK_IMAGES)]
else:
    print("No OPENAI_API_KEY - using curated Unsplash image")
    image_url = FALLBACK_IMAGES[datetime.now().weekday() % len(FALLBACK_IMAGES)]
