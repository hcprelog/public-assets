# NAICS 492110 — Courier & Express Delivery Sub Tracker
NAICS: 492110 — Couriers and Express Delivery Services
H&C Use Case: Lab/medical/pharmacy specimen courier for VA and federal medical facilities
Last updated: 2026-06-02 | Source: Web research

---

## TWO-TIER SUB STRATEGY

**Tier 1 — Execution Subs (NOT SAM/government registered — PREFERRED)**
H&C's value proposition: "We win government contracts you can never access. You do the work in your local area. We pay you Net 30 — guaranteed government money."
- They cannot bid on SDVOSB set-asides directly
- H&C has maximum leverage — we open a revenue channel they don't have
- They focus on execution; H&C handles compliance, billing, past performance

**Tier 2 — Consulting / Coaching Clients (SAM registered, not winning)**
H&C's value proposition: "You're registered but not winning because you don't have SDVOSB/HUBZone certs. We have both. Buy our GROWTH ($297/mo) or PRIME READY ($797/mo) plan — we'll get you into contracts and show you how to prime your own."
- These become subscription revenue for hcprelog.com
- May also sub on immediate bids while they build their own capability
- Long-term: they become self-sufficient govcon operators (H&C alumni)

---

## ACTIVE OPPORTUNITIES NEEDING SUBS

| Opportunity | PoP Location | Deadline | Sub Status |
|---|---|---|---|
| 36C24126Q0174 — VA VISN1 Lab Courier | Bedford, MA 01730 | Jun 15 | 2 Tier 1 candidates identified |
| 36C24626Q0536 — VA VISN6 Lab Courier | Hampton, VA (TBD) | Jun 5 🔴 | 2 candidates identified |
| 36C24926Q0230 — VA JHQVAMC Lab Courier | McDonald, TN | Jun 9 🟠 | 3 candidates identified |

---

## TIER 1 CANDIDATES — MASSACHUSETTS (Bedford/Boston area)

### Bocsit
| Field | Detail |
|---|---|
| Location | Boston, MA (serves all of Massachusetts) |
| Phone | 617-807-0411 |
| Website | bocsit.com |
| Email | No public email found — use phone or bocsit.com contact form |
| Services | Same-day medical specimen, STAT delivery, HIPAA compliant, 24/7 |
| SAM Registered | Unknown — no government work found ✅ Tier 1 |
| Size | Small (1-20 est.) |
| Status | **COLD — phone/web form outreach (no email found)** |
| Notes | Medical specimen specialist — directly relevant to VA lab courier work. Call 617-807-0411 or submit via bocsit.com contact form. Phone script below. |

### Allstate Courier Systems *(Tier 2 — consulting prospect)*
| Field | Detail |
|---|---|
| Location | 183 Whiting St Ste 6, Hingham, MA 02043 |
| Phone | 781-335-0261 |
| Email | info@allstatecouriers.com |
| Services | Medical courier, route service, STAT, 24/7, New England + national |
| SAM Registered | YES — state AND federal certified contractor ⚠️ Tier 2 |
| Certifications | WOSB (Women-Owned Small Business) |
| Status | **EMAIL SENT 2026-06-02** |
| Notes | Already government-certified but NOT SDVOSB — pitch PRIME READY plan for SDVOSB teaming access. Could also sub on immediate bids. |

---

## TIER 1 CANDIDATES — VIRGINIA (Hampton/Norfolk area)

### East Coast Courier Company (ECCC) ⭐ TOP PICK
| Field | Detail |
|---|---|
| Location | 723 Woodis Ave, Norfolk, VA 23510 |
| Phone | (757) 640-7767 |
| Email | eccconline@gmail.com |
| Website | eastcoastcourieronline.com |
| Services | Small parcel delivery, Southeast VA corridor, Bon Secours hospitals, Red Cross |
| SAM Registered | Not found ✅ Tier 1 |
| Size | Independent fleet, Hampton Roads area |
| Founded | 1999 — family-owned |
| Status | **EMAIL SENT 2026-06-02** |
| Notes | Already serves Bon Secours hospitals — VA medical work is a natural fit. Family-owned = owner-accessible. No government contracting overhead. |

### CourierVA (Ports Atlantic Logistics LLC) ⭐ VETERAN-OWNED
| Field | Detail |
|---|---|
| Location | PO Box 11301, Norfolk, VA 23517 / 452 Carl St, Norfolk, VA 23505 |
| Phone | 757-270-1239 (ask for Bruce) |
| Email | info@courierva.com |
| Services | Route courier, freight, Norfolk to Richmond, Tidewater, Hampton Roads |
| SAM Registered | Unknown — no federal contracts found ✅ Likely Tier 1 |
| Size | Small — owner-operated |
| Owner | Jon Broudy (legal); Bruce (ops contact) |
| Status | **EMAIL SENT 2026-06-02** |
| Notes | **VETERAN-OWNED** — perfect alignment with H&C. Pitch: "veteran-to-veteran partnership, H&C has SDVOSB cert you don't, we team and split the revenue." May want to help them get SDVOSB certified long-term. |

---

## TIER 1 CANDIDATES — TENNESSEE (Johnson City/Mountain Home area)

### Tri City Logistics ⭐ DIRECT CONTACT AVAILABLE
| Field | Detail |
|---|---|
| Location | 936 Eastern Star Rd, Kingsport, TN 37663 |
| Phone | (423) 279-1771 |
| Cell | (423) 502-0803 |
| Email | dispatch@tricitylogistics.com |
| Contacts | Chad Domby + Josh Olinger |
| Services | Freight carrier, Kingsport/Tri-Cities area |
| SAM Registered | Unknown — check before outreach |
| Status | **EMAIL SENT 2026-06-02** |
| Notes | Kingsport is 20 min from Mountain Home VA. Direct contact available — fastest path to outreach. |

### Tri Cities Solutions LLC
| Field | Detail |
|---|---|
| Location | East Tennessee (exact address TBD) |
| Website | tricitiessolutions.com |
| Services | Medical courier, standard courier, OSHA + HIPAA certified |
| SAM Registered | Unknown — likely not based on small footprint ✅ Likely Tier 1 |
| Status | **COLD — not yet contacted** |
| Notes | Medical specialist, HIPAA certified — exactly what VA lab courier requires. Owner contact via website form. |

### Courier Express Tri-Cities
| Field | Detail |
|---|---|
| Location | 102 L P Auer Rd, Johnson City, TN |
| Website | courierexpress.net |
| Services | Healthcare, lab, vet lab, logistics, 12K sq ft warehouse |
| SAM Registered | Likely YES — has warehouse infrastructure |
| Status | **COLD — verify SAM before outreach** |
| Notes | If SAM registered → Tier 2 consulting pitch. If not → Tier 1 sub. |

---

## BOCSIT PHONE / WEB FORM SCRIPT (Bedford MA / VISN1 bid)

**Phone opening (call 617-807-0411):**
> "Hi, my name is Dennis Hunter, President of H&C Precise Logistics — we're an SDVOSB and HUBZone certified federal contractor out of Durham, NC. I'm reaching out because we just identified a VA medical center contract for lab and specimen courier services in the Bedford, Massachusetts area. It's a veteran-owned set-aside — we win the prime contract, you run the routes locally, we pay you Net 30. You're already doing this exact type of work. I'd love 15 minutes to explain how the partnership works — is there someone I could speak with about that?"

**Web form message (bocsit.com contact form):**
> Subject: VA Hospital Lab Courier Contract — Local Partner Opportunity in Bedford, MA
>
> Hi Bocsit team,
>
> My name is Dennis Hunter, President of H&C Precise Logistics (hcprelog.com). We're an SDVOSB and HUBZone certified federal contractor based in Durham, NC.
>
> We've identified an active VA medical center contract for lab and specimen courier services in the Bedford, Massachusetts area. As a veteran-owned set-aside contract, H&C qualifies to prime it — and we're looking for a local medical courier partner to run the routes.
>
> The arrangement is straightforward: H&C holds the prime contract and handles all compliance and billing. Bocsit runs the routes as you normally would. We pay you Net 30 once the government pays us.
>
> Your 24/7 HIPAA-compliant specimen courier capability is exactly what this contract requires. I'd love a 15-minute call to walk you through it.
>
> Best,
> Dennis Hunter | President, H&C Precise Logistics | 919-275-0598 | dennis@hcprelog.com

---

## OUTREACH LOG

| Date | Company | Contact | Method | Response | Next Step |
|---|---|---|---|---|---|
| 2026-06-02 | East Coast Courier (ECCC) | eccconline@gmail.com | Email sent (GitHub Actions SMTP) | Awaiting reply | Monitor auto-checks daily |
| 2026-06-02 | CourierVA (Bruce) | info@courierva.com | Email sent (GitHub Actions SMTP) | Awaiting reply | Monitor auto-checks daily |
| 2026-06-02 | Tri City Logistics | dispatch@tricitylogistics.com | Email sent (GitHub Actions SMTP) | Awaiting reply | Monitor auto-checks daily |
| 2026-06-02 | Allstate Courier Systems | info@allstatecouriers.com | Email sent (GitHub Actions SMTP) | Awaiting reply | Monitor auto-checks daily |
| 2026-06-02 | Bocsit | 617-807-0411 / bocsit.com form | Phone/web form | PENDING | Dennis to call 617-807-0411 or submit bocsit.com form |

---

## SUB OUTREACH SCRIPT — NAICS 492110

**Phone opening:**
> "Hi [Name], my name is Dennis Hunter, I'm President of H&C Precise Logistics — we're an SDVOSB and HUBZone certified federal contractor based in Durham, NC. I'm reaching out because we just identified a VA hospital contract for lab courier services in [city]. It's set aside for veteran-owned businesses — we win it, you run the routes in your area, we pay you Net 30. I wanted to see if you'd have 15 minutes to talk about it."

**Email subject line options:**
- "VA Hospital Courier Contract — Partnership Opportunity in [City]"
- "Federal Contract Revenue Opportunity — [City] Area"
- "Government Courier Contract — Looking for Local Execution Partner"

**Key objections & responses:**
- "We don't do government work" → "You don't need to — that's our job. You just run the routes like you always do."
- "We're too small" → "That's exactly why this works. We handle the compliance and billing. You focus on the delivery."
- "We need to think about it" → "The deadline is [date]. Can we talk this week? I just need 15 minutes."

**Tier 2 pivot (if they say they're already SAM registered):**
> "That's great — you're ahead of most companies. The challenge is winning the SDVOSB and HUBZone set-asides without those certifications. That's where we can help. Have you looked at teaming agreements? I'd love to show you how we structure those."

---

## LONG-TERM SUB ROSTER (vetted — use for future bids)

| Company | States | Contact | SAM | Last Used | Rating |
|---|---|---|---|---|---|
| — | — | — | — | — | — |

---

## NOTES
- VA lab courier contracts are typically $75K–$250K/year recurring
- These contracts renew — one good sub relationship = years of repeat revenue
- Sub outreach requires approval: /ops/sops/HC-SOP-001-bid-and-vendor-sourcing.md § Step 5
- Never disclose H&C markup to subs
- Help Tier 1 subs get SAM registered once relationship is established
